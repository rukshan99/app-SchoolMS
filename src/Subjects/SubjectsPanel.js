...
export class SubjectsPanel extends Component {
	...
	getSubjects = async () => {
		...
		try {
			const subjects = await axios.get(`http://localhost:8000/api/v1/subjects`);
	...
	search = async () => {
		...
		this.setState({ loading: true });
		try {
			const res = await axios.get(`http://localhost:8000/api/v1/subjects/search/${this.state.searchText}`);
			this.setState({ loading: false });
